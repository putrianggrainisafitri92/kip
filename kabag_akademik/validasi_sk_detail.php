...
<head>
    <meta charset="UTF-8">
    <title>Detail SK KIPK - Kabag Akademik</title>
    ...
    <div class="container">
        <div class="detail-card">
            <div class="header-meta">
                <h2 class="title">Detail SK KIPK</h2>
                <?php
            ...
            <div class="info-grid">
                <div class="info-item">
                    <label>Nama SK</label>
                    <span><?= htmlspecialchars($d['nama_sk']); ?></span>
                </div>
                <div class="info-item">
                    <label>Nomor SK</label>
                    <span><?= htmlspecialchars($d['nomor_sk']); ?></span>
                </div>
                <div class="info-item">
                    <label>Tahun</label>
                    <span><?= htmlspecialchars($d['tahun']); ?></span>
                </div>
                <div class="info-item">
                    <label>Tgl Upload</label>
                    <span><?= date('d M Y', strtotime($d['tanggal_upload'])); ?></span>
                </div>
            </div>

            <div class="file-box">
                <i class="far fa-file-pdf file-icon"></i>
                <div class="file-name"><?= basename($d['file_url'] ?? 'file_sk.pdf'); ?></div>
                <?php if(!empty($d['file_url'])): ?>
                    <a href="<?= $d['file_url']; ?>" target="_blank" class="btn-view-file">
            ...
            <div class="action-footer">
                <a class="btn btn-back" href="validasi_sk.php">
                    <i class="fas fa-arrow-left"></i> Kembali
                </a>

                <?php if($status_lower == 'pending'): ?>
                    <form action="validasi_sk_proses.php" method="post" id="formApprove" style="display:inline;">
                        <input type="hidden" name="id" value="<?= $id ?>">
                        <input type="hidden" name="s" value="approved">
                        <button type="button" class="btn btn-approve" onclick="confirmApprove()">
                            <i class="fas fa-check"></i> Setujui SK
                        </button>
                    </form>
...
<div id="rejectModal" class="modal">
    <div class="modal-content">
        ...
        <form action="validasi_sk_proses.php" method="post">
            <input type="hidden" name="id" value="<?= $id ?>">
...
function confirmApprove() {
    if(confirm('Apakah Anda yakin ingin menyetujui SK ini?')) {
